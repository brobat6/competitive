...
public:
    ...
    void push(int node) {
        for(int j = 0; j < 2; j++) {
            for(int i = 0; i < 20; i++) {
                if(flip[node][i] == 1) {
                    cnt[node<<1|j][i] = sz[node<<1|j] - cnt[node<<1|j][i];
                }
                flip[node<<1|j][i] ^= flip[node][i];
            }
        }
        for(int i = 0; i < 20; i++) {
            flip[node][i] = 0;
    ...
    void XOR(int node, int start, int end, int l, int r, int x) {
        if(start == end) {
            ...
            push(node);
            int mid = (start + end)>>1;
            if(l <= mid) XOR(node<<1, start, mid, l, min(mid, r), x);
            if(r > mid) XOR(node<<1|1, mid+1, end, max(mid + 1, l), r, x);
            for(int i = 0; i < 20; i++) {
                cnt[node][i] = cnt[node<<1][i] + cnt[node<<1|1][i];
            }
        }
    ...
    long long sum(int node, int start, int end, int l, int r) {
        ...
        long long ans1 = 0;
        long long ans2 = 0;
        if(l <= mid) ans1 = sum(node<<1, start, mid, l, min(mid, r));
        if(r > mid) ans2 = sum(node<<1|1, mid + 1, end, max(mid + 1, l), r);
        return ans1 + ans2;
    }
};
