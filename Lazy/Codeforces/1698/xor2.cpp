...
void build_y(vector<vector<int>> &a, int vx, int lx, int rx, int vy, int ly, int ry) {
    if (ly == ry) {
        if (lx == rx)
            t[vx][vy] = a[lx][ly];
        else
...
void build_x(vector<vector<int>> &a, int vx, int lx, int rx) {
    if (lx != rx) {
        int mx = (lx + rx) / 2;
